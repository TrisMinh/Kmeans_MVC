package model.BO;

import java.util.Arrays;
import java.util.List;
import java.util.Random;

import model.Bean.DiemBean;

public class KmeansProcBO {
	private final int K;
	private final int tonggt;
	private final int tongdiem;
	private float[][] diem;
	private float[][] tam;
	private int[] idNhom;

	public KmeansProcBO(int K, int tongdiem, int tonggt) {
		this.K = K;
		this.tongdiem = tongdiem;
		this.tonggt = tonggt;
	}

	private void chuanBiDuLieu(List<DiemBean> ds) {
		diem = new float[tongdiem][tonggt];
		idNhom = new int[tongdiem];
		Arrays.fill(idNhom, -1);
		for (int i = 0; i < tongdiem; i++) {
			for (int j = 0; j < tonggt; j++) {
				diem[i][j] = (float) ds.get(i).getGt(j);
			}
		}
		tam = new float[K][tonggt];
	}

	private static float kc(float[] a, float[] b) {
		float dx = a[0] - b[0], dy = a[1] - b[1], dz = a[2] - b[2];
		return (float) Math.sqrt(dx * dx + dy * dy + dz * dz);
	}

	public KMeansResult run(List<DiemBean> ds) {
		chuanBiDuLieu(ds);

		Random rand = new Random();
		boolean[] used = new boolean[tongdiem];

		for (int k = 0; k < K; k++) {
			int idx;
			do {
				idx = rand.nextInt(tongdiem);
			} while (used[idx]);
			used[idx] = true;
			System.arraycopy(diem[idx], 0, tam[k], 0, tonggt);
			idNhom[idx] = k;
		}

		int lap = 0;
		boolean thayDoi;

		do {
			thayDoi = false;
			float[][] tong = new float[K][tonggt];
			int[] dem = new int[K];
			for (int i = 0; i < tongdiem; i++) {
				float kcMin = kc(diem[i], tam[0]);
				int idNew = 0;
				for (int k = 1; k < K; k++) {
					float kcTam = kc(diem[i], tam[k]);
					if (kcTam < kcMin) {
						kcMin = kcTam;
						idNew = k;
					}
				}

				int idOld = idNhom[i];
				if (idOld != idNew) {
					idNhom[i] = idNew;
					thayDoi = true;
				}

				for (int j = 0; j < tonggt; j++) {
					tong[idNew][j] += diem[i][j];
				}
				dem[idNew]++;
			}
			for (int k = 0; k < K; k++) {
				if (dem[k] == 0)
					continue;
				for (int j = 0; j < tonggt; j++) {
					tam[k][j] = tong[k][j] / dem[k];
				}
			}
			lap++;
		} while (thayDoi && lap < 50);

		boolean converged = !thayDoi;
		float[][] centers = new float[K][tonggt];
		for (int k = 0; k < K; k++) {
			System.arraycopy(tam[k], 0, centers[k], 0, tonggt);
		}
		int[] labels = Arrays.copyOf(idNhom, idNhom.length);
		return new KMeansResult(labels, centers, lap, converged);
	}

	public static class KMeansResult {
		public final int[] labels;
		public final float[][] centers;
		public final int iterations;
		public final boolean converged;

		public KMeansResult(int[] labels, float[][] centers, int iterations, boolean converged) {
			this.labels = labels;
			this.centers = centers;
			this.iterations = iterations;
			this.converged = converged;
		}
	}
}

